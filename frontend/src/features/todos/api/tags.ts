...
export function useUpdateTag() {
  return useMutation({
    ...
    onError: (err: any) => {
      toast.error(err?.response?.data?.detail || "Failed to update tag");
    },
  });
}
...
export function useBulkUpdateStatus() {
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["todos"] });
      toast.success(data.message || "Bulk status updated!");
    },
    onError: () => {
      toast.error("Failed to update status for selected todos");
    },
  });
}
